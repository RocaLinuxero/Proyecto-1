...
class App:
    ...
    def modo_administrador(self):
        ...
        # Borrar Configuración de Correo
        Button(self.root, text="🗑️ Borrar Credenciales de Correo", 
               command=self.borrar_config_correo, bg="#e74c3c", fg="white").pack(pady=5)
    ...
    def borrar_config_correo(self):
        # Confirmamos con el usuario para evitar borrados accidentales
        confirmar = messagebox.askyesno("Confirmar Borrado", 
            "¿Estás seguro de que deseas eliminar las credenciales del correo?\n"
            "Esto desactivará el envío de facturas hasta que se configuren de nuevo.")
        
        if confirmar:
            # Sobrescribimos el archivo con datos vacíos
            guardar_config_correo({"remitente": "", "password": ""})
            
            registrar_bitacora("Configuración de correo eliminada", self.usuario_actual)
    ...
    def generar_factura(self):
        ...
        if not re.match(r"^[a-zA-ZáéíóúÁÉÍÓÚñÑ\s]+$", nom):
            ...
            return
        if not re.match(r"^(cf|CF|Cf|\d+)$", nit):
