...
class App:
    ...
    def generar_factura(self):
        try:
            consumo = float(self.entries["consumo"].get())
        except:
            messagebox.showerror("Error", "Consumo inválido")
            return

        correlativo = self.entries["correlativo"].get()
        nombre = self.entries["nombre"].get()
        nit = self.entries["nit"].get()
        contador = self.entries["contador"].get()
